...
def render_layout_config():
    with st.sidebar.expander("⚙️ 库位布局与自动生成", expanded=False):
        ...
        prefix = st.text_input("命名前缀 (如: A, B)", value="A")
        ...
        if st.button("✨ 一键生成布局"):
            ...
            if "按行" in gen_mode:
                for r in range(int(rows)):
                    for c in range(int(cols)):
                        ...
                        # 引入物理通道：每两列之间增加额外的 40px 间距
                        channel_gap = (c // 2) * 40
                        x = 20 + c * (width + gap_x) + channel_gap
                        y = 20 + r * (height + gap_y)
                        new_slots.append({
                            "id": f"slot-{idx}",
                            "code": code,
            ...
            save_warehouse_layout("default", {"slots": new_slots})
            st.success("布局已生成！")
            time.sleep(0.5)
            st.rerun()
...
def render_warehouse_map(mode="dashboard", pending_sn=None, fullscreen=False):
    ...
    selected_code = _warehouse_map(slots=slots_data, fullscreen=fullscreen, key=f"warehouse_map_comp_{mode}")
    
    if selected_code and selected_code != st.session_state.get('last_selected_code'):
